...
| 항목 | 값 |
|---|---|
| 카메라 | Zivid (모델 미확정 — 미결 질문 참조) |
| 해상도 | 1224 x 1024 (2x2 subsample) — 2D/3D 동일 |
| 로봇 | UR3e (페이로드 3kg) |
| 엔드이펙터 | 석션 전제 (컵 직경 미확정, 기본값 15mm) |
...
## 8. 미결 질문 (코드 작성 전 확인 필요)

**필수**

1. **GPU 모델 / VRAM** — SAM2 체크포인트 선택 기준. `sam2.1_hiera_small`(~180MB) vs `large`(~900MB)
2. **인터넷 접근 가능 여부** — SAM2 체크포인트, HuggingFace `grounding-dino-tiny` 다운로드 필요. 사내망 프록시 차단 시 별도 경로 필요
3. **카메라 설치 각도** — 수직 하향인지 기울여 설치인지. 거의 수직이면 bin frame 로직 단순화 여지 있음
4. **빈 내부 깊이(mm)** — 림 → 바닥 오프셋 계산용

**기본값으로 진행 가능**

5. 카메라 모델 (Zivid 2+ M60 / L110 등) — 노이즈·포인트 밀도 기대치
6. 석션컵 직경 — 기본값 15mm
7. m2 텍스트 프롬프트 어휘 — 기본값 `"pouch. box. bottle. tube. can. bag."`
8. 목표 사이클 타임
